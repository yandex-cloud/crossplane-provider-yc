...
type ServiceAccountAPIKeyInitParameters struct {
	...
	// (String). The scope of the key. Use lifecycle {ignore_changes = [scope]} directive to avoid false changes on apply.
...
type ServiceAccountAPIKeyObservation struct {
	...
	// Only) (String). The encrypted secret key, base64 encoded. This is only populated when pgp_key is supplied.
...
// ServiceAccountAPIKey is the Schema for the ServiceAccountAPIKeys API. Manages the yandex_iam_service_account_api_key resource.
