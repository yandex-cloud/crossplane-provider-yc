...
import (
	...
	orig "github.com/crossplane/upjet/pkg/controller"
...
type TerraformPluginFrameworkConnector struct {
	...
	operationTrackerStore       *orig.OperationTrackerStore
...
func NewTerraformPluginFrameworkConnector(kube client.Client, sf terraform.SetupFn, cfg *config.Resource, ots *orig.OperationTrackerStore, opts ...TerraformPluginFrameworkConnectorOption) *TerraformPluginFrameworkConnector {
...
func (n *terraformPluginFrameworkExternalClient) Observe(ctx context.Context, mg xpresource.Managed) (managed.ExternalObservation, error) { //nolint:gocyclo
	...
	if fatalDiags := getFatalDiagnostics(readResponse.Diagnostics); fatalDiags != nil {
		errStr := fatalDiags.Error()
		if strings.HasSuffix(errStr, "not found") {
			state, err := tfprotov5.NewDynamicValue(n.resourceValueTerraformType, tftypes.Value{})
			if err != nil {
				return managed.ExternalObservation{}, errors.Wrap(err, "totally unexpected error")
			}
			readResponse.NewState = &state
			readResponse.Diagnostics = []*tfprotov5.Diagnostic{}
			readResponse.Private = nil
		} else {
...
func (n *terraformPluginFrameworkExternalClient) setExternalName(mg xpresource.Managed, stateValueMap map[string]interface{}) (bool, error) { //nolint:interfacer
